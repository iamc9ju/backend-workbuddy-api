...
func loadConfig() {
	viper.SetConfigName(".env") // ชื่อไฟล์ โดยไม่ใส่นามสกุล
	viper.SetConfigType("env")  // กำหนดว่าเป็นไฟล์ .env

	viper.AddConfigPath("./")     // มองในโฟลเดอร์ src/
	viper.AddConfigPath("../")    // มองที่โปรเจกต์ root
	viper.AddConfigPath("../../") // เผื่อใช้ใน test context

	if err := viper.ReadInConfig(); err == nil {
		utils.Log.Infof("✅ Config file loaded from %s", viper.ConfigFileUsed())
		return
	}
